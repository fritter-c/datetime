...
namespace gtr{
    ...
    struct datetime{
        ...
        bool
        to_string_format(char* out,const char* format = DATETIME_DEFAULT_FORMAT, date_format group_format = date_format::text_date) const;
    ...
    struct year_field{
        static inline int parse(const char** state, datetime_pack& pack){
            char buffer[8];
            int index = 0;
            while(_IS_ALPHA(**state)){buffer[index++] = *(*state)++;}
            buffer[index] = '\0';
        ...
        static inline void puts(const char** format,char** out,datetime_pack& pack){
            if (*(++*format) == 'Y'){
                ...
                //Two digits
                    ...
                    int year = pack.year;
                    int factor = 1;
                    for(;digits - 2 > 0;digits--){factor*=10;}
    ...
    struct microsecond_field{
        static inline int parse(const char **state, datetime_pack &pack) {
            ...
            for(int i = 0; i < 6 - index; i++){pack.microsecond*=10;};
            return index;
        }
        static inline void puts(const char** format, char** out, datetime_pack& pack){
            int digits = 1;
            while(*(++*format) == 'z') {digits++;}
